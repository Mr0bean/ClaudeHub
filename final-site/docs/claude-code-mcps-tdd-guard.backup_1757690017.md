...
3.  选择 **+ 添加新匹配器...** 并输入：`Write|Edit|MultiEdit|TodoWrite`
4.  选择 **+ 添加新钩子...** 并输入：`tdd-guard`
5.  选择保存位置（推荐项目设置）

请参阅[强化 TDD 执行](https://github.com/nizos/tdd-guard)以防止代理绕过验证。
...
TDD Guard 作为独立进程运行，并根据 TDD 原则验证文件修改。它拦截 `Write`、`Edit` 和 `MultiEdit` 操作，检查三种主要 TDD 违规：
...
请注意，TDD Guard 可能会给你的 `Write`、`Edit`、`MultiEdit` 和 `TodoWrite` 操作引入延迟，因为它会根据 TDD 原则验证每个操作。
